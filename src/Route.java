...
public class Route {
    ...
    public double getWeariness() {
		...
		for ( int j = 0; j < size; j++) {
			weight += StoresGift.GetWeight(this.gifts.get(j)); // Add weight of all gifts
		}

    	// Compute weariness from north pole to the first destination
    	distance = StoresGift.GetDistance(this.gifts.get(0), northPole);
    	weariness = distance * weight;
    	
    	// Compute weariness for all destinations
    	for (int i = 1; i < size; i++) {
    		weight = weight - StoresGift.GetWeight(this.gifts.get(i)); // Remove weight of the gift of the previous destination
    		distance = StoresGift.GetDistance(this.gifts.get(i-1), this.gifts.get(i));
